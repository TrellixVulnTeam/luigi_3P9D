...
function parse_assign() {
    ...
    if (match(token_types.EQUAL)) {
        ...
        parse_expression();
        emit(variable.global ? 'store_global' : 'store_local', name);
    } else if (match(token_types.LEFT_BRACKET)) {
        if (variable == null)
            error(start, `Variable "${name}" does not exist`);

        emit(variable.global ? 'load_global' : 'load_local', name);
    ...
    while (offset < tokens.length) {
        ...
        if (tok.type == token_types.BOOLEAN ||
            ...
            if (tokens[offset + 1].type == token_types.LEFT_PARENTHESIS) {
                ...
                emit(variable.global ? 'load_global' : 'load_local', tok.value);
            }
...
function find_operator(type, want_op) {
    if (want_op) {
        switch (type) {
            case token_types.OR:               return { priority: 0, code: 'or', right: false };
            case token_types.AND:              return { priority: 1, code: 'and', right: false };
            case token_types.LESS:             return { priority: 3, code: 'less', right: false };
            case token_types.LESS_OR_EQUAL:    return { priority: 3, code: 'less_or_equal', right: false };
            case token_types.GREATER:          return { priority: 3, code: 'greater', right: false };
            case token_types.GREATER_OR_EQUAL: return { priority: 3, code: 'greater_or_equal', right: false };
            case token_types.EQUAL:            return { priority: 3, code: 'equal', right: false };
            case token_types.NOT_EQUAL:        return { priority: 3, code: 'not_equal', right: false };
            case token_types.PLUS:             return { priority: 4, code: 'add', right: false };
            case token_types.MINUS:            return { priority: 4, code: 'substract', right: false };
            case token_types.MULTIPLY:         return { priority: 5, code: 'multiply', right: false };
            case token_types.DIVIDE:           return { priority: 5, code: 'divide', right: false };
        }
    } else {
        switch (type) {
            case token_types.PLUS:  return { priority: 6, code: null, right: true };
            case token_types.MINUS: return { priority: 6, code: 'negate', right: true };
            case token_types.NOT:   return { priority: 2, code: 'not', right: true };
        }
    }
}
