...
function parse_if() {
    ...
    if (match('then')) {
        if (tokens[offset].type == 'identifier' && tokens[offset + 1].type == '=') {
            parse_declaration();
...
const binary_operators = {
    'and': { priority: 1, code: 'and' },
    'or': { priority: 2, code: 'or' },
    '<': { priority: 3, code: 'less' },
    '<=': { priority: 3, code: 'less_or_equal' },
    '>': { priority: 3, code: 'greater' },
    '>=': { priority: 3, code: 'greater_or_equal' },
    '=': { priority: 3, code: 'equal' },
    '+': { priority: 4, code: 'add' },
    '-': { priority: 4, code: 'substract' },
    '*': { priority: 5, code: 'multiply' },
    '/': { priority: 5, code: 'divide' }
};

const unary_operators = {
    '+': { priority: 0, code: null },
    '-': { priority: 0, code: 'negate' },
    'not': { priority: 0, code: 'not' }
};
...
function parse_expression() {
    ...
    while (offset < tokens.length) {
        ...
        if (tok.type == 'boolean' ||
            ...
            let op = binary_operators[tok.type];
            ...
            while (pending_operators.length) {
                ...
                if (pending == '(')
                    break;
                if (pending.priority <= op.priority)
                    break;
            ...
            let op = unary_operators[tok.type];
