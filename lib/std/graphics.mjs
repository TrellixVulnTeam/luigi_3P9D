...
export function set_fullscreen(fullscreen) {
    let full = raylib.FLAG_WINDOW_UNDECORATED |
               raylib.FLAG_WINDOW_MAXIMIZED |
               raylib.FLAG_WINDOW_TOPMOST;
    let windowed = raylib.FLAG_WINDOW_RESIZABLE;
...
export function load_image(path) {
...
export function is_mouse_down(btn) {
    return raylib.IsMouseButtonDown(btn);
}

export function is_mouse_pressed(btn) {
    return raylib.IsMouseButtonPressed(btn);
}

export function get_mouse_pos() {
    let pos = {
        x: raylib.GetMouseX(),
        y: raylib.GetMouseY()
    }
