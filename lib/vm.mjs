...
export function execute(_functions, _globals = {}) {
    ...
    run_function(default_func, globals, 0);
}

function run_function(func, locals, depth) {
    let values = [];
    let addr = 0;
    ...
    try {
            ...
            switch (inst.code) {
                ...
                case opcodes.CALL: {
                    ...
                    if (func.instructions != null) {
                        ...
                        let value = run_function(func, params, depth + 1);
                        values.push(value);
                    } else {
                ...
                case opcodes.CALL_INDIRECT: {
                    ...
                    if (inst.value != func.params.length)
                        throw new Error(`Function "${func.name}" expects ${func.params.length} arguments, not ${inst.value}`);
        ...
        let where = (func.name != '.') ? func.name + '()' : '<script>';
        let line = func.instructions[addr].line;

        if (!err.message.includes('\n'))
            err.message += '\n\nStack trace:';
        err.message += `\n    ${where.padEnd(24, ' ')} line ${line}`;
