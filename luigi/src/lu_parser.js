...
exports.parse = function(file) {
    ...
    import_natives(functions, std_raylib);
    ...
    if (offset < tokens.length)
        error(offset, `Unexpected token ${token_typeS.get_name(tokens[offset].type)}, expected end of file`);
...
function parse_code(ender) {
    while (offset < tokens.length) {
        let tok = tokens[offset];
        line = tok.line;

        if (tok.type == token_types.END || tok.type == ender)
            break;
